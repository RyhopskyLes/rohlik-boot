...
public class CategoryKosik {
	...
	@ManyToMany(cascade = {
    	...
    	@JoinTable(name="kosik_rohlik_category",
    	    joinColumns = @JoinColumn(name = "kosik_id", referencedColumnName="id"),
    	    inverseJoinColumns = @JoinColumn(name="rohlik_id", referencedColumnName="id")
    	)
	@JsonIgnore
	...
	public CategoryKosik doIf(Boolean condition, UnaryOperator<CategoryKosik> operate) {
		if(condition) {CategoryKosik temp = operate.apply(this);
		return temp;}
		return this;	
	}
	...
	public String toString() {
		return "CategoryKosik [id=" + id + ", categoryName=" + categoryName + ", uri=" + uri + ", equiId=" + equiId
				+ ", equiCategoryName=" + equiCategoryName + ", parentName=" + parentName +", parentUri=" + parentUri + ", equiParentId="
				+ equiParentId +", active=" + active + "]";
	}
